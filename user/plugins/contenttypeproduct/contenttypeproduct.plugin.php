 ...
 *
 * A plugin for Habari that enables the "Product"
 * content-type.
 *
 * @package contenttypeproduct
 */

 class ContentTypeProduct extends Plugin 
{ 
    ...
    // add the content type.
		...
		$group->grant('post_product', 'read');
	}
	
	public function action_plugin_deactivation( $plugin_file ) {
		Post::deactivate_post_type( 'product' );
	}
	...
	public function action_init() {		
		// Create templates
		$this->add_template('product.single', dirname(__FILE__) . '/product.single.php');
	}
	...
	public function action_form_publish($form, $post, $context) {
	  // only edit the form if it's an event
		if ($form->content_type->value == Post::type('product')) {
      // just want to add a text field
      $form->insert('tags', 'text', 'address', 'null:null', _t('Product Address'), 'admincontrol_textArea');
	...
	public function action_publish_post( $post, $form ) {
		if ($post->content_type == Post::type('product')) {
